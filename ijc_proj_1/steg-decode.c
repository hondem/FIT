...
int main(int argc, char *argv[]){
    if(argc != 2) error_exit("Bad arguments passed!");

    struct ppm *picture;
    picture = ppm_read(argv[1]);

    bit_array_create(prime_numbers, 3000000L); // Tento limit je nastaven, jelikož maximální možné rozlišení obrázku je 1000x1000
    ...
    for (unsigned long i = 11; i < 3000000L; i++) {
