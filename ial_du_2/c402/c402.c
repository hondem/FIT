...
**/
	...
	/* Trocha magie, snad to bude fungovat? O.o */
	tBTNodePtr *tmp = RootPtr;

	while((*tmp) != NULL){
		if((*tmp)->Cont > Content) tmp = &((*tmp)->LPtr); // Klíč je menší -> musíme jít doléééva
		else if((*tmp)->Cont < Content) tmp = &((*tmp)->RPtr); // Klíč je větší -> musíme jít doprááááva
		else return; // Klíč je roven Content -> Nic nedělat!
	}
...
**/
	...
	Leftmost_Postorder(tmp, &stackPointers, &stackBools);

	while(!SEmptyP(&stackPointers)) {
		tmp = STopPopP(&stackPointers);
		SPushP(&stackPointers, tmp);

		if(!STopPopB(&stackBools)){
			STopPopP(&stackPointers);
			BTWorkOut(tmp);
		} else {
			SPushB(&stackBools, false);
			Leftmost_Postorder(tmp->RPtr, &stackPointers, &stackBools);
		}
	}
}
...
**/
	...
	tBTNodePtr currentNode = *RootPtr;
	SPushP(&stack, currentNode);
	while(!SEmptyP(&stack)){
		currentNode = STopPopP(&stack);
		if(currentNode->LPtr != NULL) SPushP(&stack, currentNode->LPtr);
		if(currentNode->RPtr != NULL) SPushP(&stack, currentNode->RPtr);
		free(currentNode);
	}
